...
def test_unauthorized_edit_page(client: TestClient) -> None:
...
def test_edit_page(client: TestClient) -> None:
    with session_maker() as session:
...
def test_delete_endpoint_unauthorized_response(client: TestClient) -> None:
    response = client.get("/admin/movie/delete/", follow_redirects=False)
    assert response.status_code == 302

    response = client.get("/admin/movie/delete/1")
    assert response.status_code == 403


def test_delete_endpoint_not_found_response(client: TestClient) -> None:
    response = client.get("/admin/user/delete/1")
...
def test_delete_endpoint(client: TestClient) -> None:
    ...
    response = client.post("/admin/user/delete/1", follow_redirects=False)
...
def test_edit_get_page(client: TestClient) -> None:
    with session_maker() as session:
...
def test_edit_submit_form(client: TestClient) -> None:
    with session_maker() as session:
        user = User(name="Joe")
        session.add(user)
        session.flush()
...
def test_export_json_complex_model(client: TestClient) -> None:
    ...
    assert response.text == json.dumps([{"id": "1", "user_id": "1", "user.profile.id": "None", "user": "User 1"}])
