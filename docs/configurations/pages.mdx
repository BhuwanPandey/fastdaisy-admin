...
Two filters are available which get implemented automatically based on model fields `for_eg: column_filters = [User.name, "is_admin"]` but you can also create custom filters, by following [sqladmin docs](https://aminalaee.github.io/sqladmin/configurations/#columnfilter).
